...
## What leaves the host is redacted

The local archive and the off-host copy are deliberately not the same bytes.

A local bundle sits on the machine that already holds these secrets, so it stays
complete: it is the copy that restores a working install. An upload crosses into object
storage, where every principal who can read the bucket can read the bundle, so the copy
that crosses is rewritten first — both mandatory outbound redactors run over it, and
`config.json`'s token, plus anything credential-shaped in a note or a memory row, is
replaced with an inert tag.

Two consequences worth knowing before you rely on it:

- **Restoring the off-host copy gives you working memory and inert credentials.** The
...
object — so a database in that state is one this pass cannot clean: the product's own is
left out and reported, and yours is kept while the upload refuses.
...
moving is one that cannot be shown clean, so it is left out and reported rather than
uploaded.
...
a database the product ships is treated the same way: if it cannot be read as SQLite it
is kept and the upload refuses, where the product's own unreadable database is left out
and reported.

Set `redact_backup_uploads` to `false` to upload the complete bundle instead. That makes
the off-host copy restore a working install, and it puts usable credentials in the
bucket — a deliberate trade, not a default. If redaction is on and cannot be completed,
the upload is refused; it never falls back to sending the unredacted bundle.
