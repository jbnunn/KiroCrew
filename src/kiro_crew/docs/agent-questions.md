...
`needs_input`. The map is bounded (`_MAX_PENDING_QUESTIONS`, oldest evicted) since
only the presence of an entry drives the status.
...
| Path | Retires |
|---|---|
| a LIVE `user`-role message (composer, queue, nudge, channel relay) | the stateless entry only |
