...
class DashboardState:
    ...
    # Unanswered-question records kept per slot (see mark_question_pending). Only
    # the presence of any entry drives the status, so this is a memory bound
    # rather than a product limit: overlapping asks are a handful at most, while a
    # loop posting cards with no user message in between is unbounded.
    _MAX_PENDING_QUESTIONS = 16
    ...
    def mark_question_pending(
        ...
        *questions* (already redacted) is stored for a STATELESS card so a
        ...
        # Bounded: entries retire on answer, dismissal, or the next user message,
        # but an agent that posts card after card with no message in between would
        # otherwise grow this without limit. The status only needs to know that
        # SOMETHING is unanswered, so dropping the oldest costs nothing a surface
        # reads — and the dropped entry's own retire call is a harmless no-op.
        while len(slot._question_pending) > self._MAX_PENDING_QUESTIONS:
            slot._question_pending.pop(next(iter(slot._question_pending)))
        self._push_slots()
