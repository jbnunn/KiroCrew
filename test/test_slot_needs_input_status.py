...
def test_the_record_map_is_bounded() -> None:
    """Many parked asks on one slot must not grow the map without limit.

    Blocking entries are the unbounded case: each one is separately answerable, so
    they legitimately coexist (a stateless card supersedes its predecessor and can
    only ever hold one entry).
    """
    st = _state("chat-1")
    cap = DashboardState._MAX_PENDING_QUESTIONS
    ...
    # The OLDEST are dropped: the status only needs to know something is unanswered.
    assert "ask-0" not in pending
    assert f"ask-{cap + 4}" in pending
