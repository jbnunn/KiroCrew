...
class TestARollbackCopyReproducesTheTree:
    ...
    def test_no_rollback_save_uses_the_link_dropping_copy(self) -> None:
        src = Path(snap.__file__).read_text(encoding="utf-8")
        offenders = re.findall(r"_copytree_safe\([^)]*backup[^)]*\)", src)
        assert offenders == [], f"rollback save still drops links: {offenders}"
