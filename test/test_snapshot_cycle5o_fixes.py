...
class TestTheManifestIsRedactedToo:
    ...
    def test_it_stays_valid_json_so_the_bundle_stays_restorable(
        self, tmp_path: Path
    ) -> None:
    ...
    def test_a_scrub_that_would_break_the_json_refuses(
        ...
        stage = _stage(tmp_path, {"components": {}, "home": f"/home/{KEY}/crew"})
        monkeypatch.setattr(
            redact, "_scrub", lambda text: (text.replace(KEY, '"'), 1) if KEY in text
            else (text, 0)
        )
...
class TestATriggerCannotPutBackWhatWasRemoved:
    def test_a_trigger_copying_the_old_value_is_still_cleaned(
        self, tmp_path: Path
    ) -> None:
    ...
    def test_a_database_that_never_settles_is_refused(self, tmp_path: Path) -> None:
        ...
        assert "memory.db" in report.dropped
        assert any("did not settle" in s for s in report.skipped_unreadable)
    ...
    def test_the_products_own_fts_triggers_do_not_trip_it(self, tmp_path: Path) -> None:
        ...
        with redact.sqlite3.connect(str(db)) as conn:
            ...
            assert conn.execute(
                "SELECT count(*) FROM items_fts WHERE items_fts MATCH 'key'"
            ).fetchone()[0] == 1, "ordinary search stopped working"
        conn.close()
