...
describe('PrList — the respond control', () => {
  ...
  it('refuses when the cached hint says ready but the fresh read disagrees', async () => {
    ...
    // And it does not silently do nothing: the row explains itself.
    await waitFor(async () =>
      expect((await control()).getAttribute('title')).toMatch(/Settings|no longer usable/i))
  })

  it('refuses when the fresh readiness read fails outright', async () => {
